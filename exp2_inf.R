...
source_folder_adjmatrix<-"adj_matrix/exp2_inf/"
folder_edgelist<-"edge_lists/exp2_inf/"
...
truncationtime_10f<-42000 #for 70 mins
for (i in 1:length(selected_colonies)){
...
folder_path_networkpara <-"network_parameters/exp2_inf/"
output_folder <- "network_parameter_plots/exp2_inf/"
...
source_folder_networkpara <-"network_parameters/exp2_inf/"
folder_path_networkplots <-"network_parameter_plots/exp2_inf/"
folder_path_qqplots<-"network_parameter_plots/exp2_inf/STATS/qqplots/"
...
strength_singleant<-read.csv(file = "network_parameters/exp1_12hs/ANTLEVEL_strength_mean05022026.csv")
...
modeltreatment <- glmmTMB(
  strength ~ treatment + (1 | colony),
  data = df_glmm,
  family = gaussian()# 'ensures is positive', right skewed (link = log)
) 

modeltreatment <- glmmTMB(
  strength ~ genotype*puremixed + (1 | colony),
  data = df_glmm,
  family = gaussian()# 'ensures is positive', right skewed (link = log)
) 
